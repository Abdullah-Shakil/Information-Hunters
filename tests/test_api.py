...
def test_health_is_open_and_data_routes_need_the_token(client):
    assert client.get("/health", headers={"X-Test-No-Auth": "1"}).status_code == 200
    assert client.get("/leads", headers={"X-Test-No-Auth": "1"}).status_code == 401
    assert client.get("/fleet", headers={"X-Test-No-Auth": "1"}).status_code == 401
    assert client.get("/leads").status_code == 200
    fleet = client.get("/fleet")
    assert fleet.status_code == 200
    assert "scrapers" in fleet.json()
