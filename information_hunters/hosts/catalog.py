...
CATALOG: tuple[ProviderSpec, ...] = (
    ProviderSpec(
        ...
        kind="host",
        controllable=True,
        free_limits="2,000 minutes/month on private repositories. Minutes for public repositories are free. The shipped workflow runs one hunt batch (30 minute timeout) and is scheduled every 15 minutes. GitHub still ends any single job after 6 hours.",
        summary="Start dispatches .github/workflows/cloud-worker.yml and turns the 15-minute schedule on. Stop cancels the current run and turns that schedule off.",
        setup_hint="Create a personal access token that can dispatch and cancel Actions, and write Actions variables, on the repository that contains this project.",
        setup_url="https://github.com/settings/tokens",
        manual_setup="Once: put this repo on GitHub. Add Actions secrets DATABASE_URL (Supabase session-pooler URI, or leave it empty and set SUPABASE_DB_URL) and SECRETS_MASTER_KEY (same value as the website). The workflow is .github/workflows/cloud-worker.yml. It runs on a 15-minute schedule unless the repository variable CLOUD_WORKER_ENABLED is false.",
        keeps_running="Start runs the workflow now and sets CLOUD_WORKER_ENABLED=true so the schedule keeps claiming hunts. Stop cancels the current run and sets that variable to false. If the token cannot write variables, Stop only cancels the current run and the cron can start another within 15 minutes.",
        fields=(
            ...
            _f("GITHUB_REPO", "Repository name", "The repo name only, for example Information-Hunters. Environment GITHUB_REPO=owner/name is split for you."),
            _f("GITHUB_WORKFLOW", "Workflow file", "Shipped as cloud-worker.yml. Change this only if you renamed the file.", default="cloud-worker.yml"),
