...
export default function FleetAgents({ kind }: Props) {
  ...
  async function startCloud() {
    ...
    try {
      ...
      setActionMsg("Cloud worker started. The 15-minute schedule is on, so hunts keep running if this PC sleeps.");
      await load();
    } catch (err) {
  ...
  async function stopCloud() {
    ...
    try {
      ...
      setActionMsg("Cancelled the current run and paused the 15-minute schedule until you press Start.");
      await load();
    } catch (err) {
