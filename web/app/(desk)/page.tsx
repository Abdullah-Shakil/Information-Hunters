  ...
  return (
    <>
      <header className="page-head">
        ...
        <Link className="btn primary" href="/hunts">
          New hunt
        </Link>
      </header>
      {error ? <p className="error">{error}</p> : null}
      <section className="stats">
        <article className="stat"><span>Qualified leads</span><strong>{stats?.leads ?? "–"}</strong></article>
        <article className="stat"><span>New this week</span><strong>{stats?.new_leads_7d ?? "–"}</strong></article>
        <article className="stat"><span>No website</span><strong>{stats?.no_website ?? "–"}</strong></article>
        <article className="stat"><span>Active hunts</span><strong>{stats?.active_jobs ?? "–"}</strong></article>
      </section>
      <div className="split">
        ...
        <section className="panel">
          ...
          {jobs.map((job) => (
            <div key={job.id} style={{ padding: "10px 0", borderTop: "1px solid var(--line)" }}>
